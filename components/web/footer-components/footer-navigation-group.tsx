...
interface Props {
  title: string;
  links: { lable: string; href: string }[];
      ...
      <div className="flex flex-col sm:gap-6 sm:mt-10 gap-4 mt-6">
          {links.map(({ lable, href }) => (
            <div key={href} className="flex items-center gap-2">
                ...
                <Link href={href} className="font-medium opacity-80 sm:text-base text-sm line-clamp-1 hover:ps-1 hover:text-primary hover:opacity-100 transition-all duration-200" >{lable}</Link>
            </div>
          ))}
      </div>
    </div>
